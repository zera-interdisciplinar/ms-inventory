...
@Service
public class UpdateRuleTargetImpl implements UpdateRuleTarget {
    private final RuleRepository ruleRepository;

    public UpdateRuleTargetImpl(RuleRepository ruleRepository) {
        this.ruleRepository = ruleRepository;
    }

    @Override
    public Rule execute(UUID id, RuleTargetType targetType, UUID targetId) {
        Rule rule = ruleRepository.findById(id)
                .orElseThrow(() -> new RuleNotFoundException(id));
        rule.changeTarget(targetType, targetId);
